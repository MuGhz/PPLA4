...
class OrderTest extends TestCase
{
    ...
    {
        $map = $map = [
            ["query",null,"PATTIMURA"],
        ];
        $request = $this->requestMockWithSession($map, false, $this->airport);

        $order = $this->curlMock($this->airport);
        $return = $order->getAirport($request);
        $this->assertEquals('{"query":"airports","suggestions":[{"value":"Ambon (AMQ), PATTIMURA","data":"AMQ"}]}', json_encode($return));
    }

    public function testGetAirportListWithSession()
    {
        $map = $map = [
            ["query",null,"PATTIMURA"],
        ];
        $request = $this->requestMockWithSession($map, true, $this->airport);
